...
if ($InstallGit) {
    ...
    if (Get-Command winget -ErrorAction SilentlyContinue) {
        ...
        Write-Host "[ERROR] winget is not available on this system. Please install Git manually." -ForegroundColor Red
        exit 1
    }
...
if ($VSSelectionAction -ne "Skip" -and $FinalVSPath) {
    ...
    if (Test-Path $VsWherePath) {
        ...
        if ($IsVerified) {
            ...
            Write-Host "[WARNING] Workloads from .vsconfig could not be verified for path: $FinalVSPath" -ForegroundColor Yellow
            Write-Host "Please contact the IT Help Desk, or submit an Issue to GitHub." -ForegroundColor Yellow
        }
    } else {
...
# --- 2.5 Fetch Ninja via vcpkg ---
Write-Host "Setting up Ninja build tool..." -ForegroundColor Yellow
$VcpkgExe = Join-Path $VcpkgPath "vcpkg.exe"

# This forces vcpkg to download its internal copy of ninja if it hasn't already
& $VcpkgExe fetch ninja | Out-Null

# Extract the tool path from vcpkg
$NinjaDir = Split-Path (& $VcpkgExe fetch ninja) -Parent

# Add Ninja to the current PowerShell session's environment path so CMake can find it immediately
$env:Path = "$NinjaDir;" + $env:Path
Write-Host "Ninja successfully configured at: $NinjaDir" -ForegroundColor Green
